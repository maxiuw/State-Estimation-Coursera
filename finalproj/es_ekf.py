...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    # p_cov here is a position so 
    # 3.1 Compute Kalman Gain
    # different that in nonlinear state *
    # print(h_jac @ p_cov_check @ h_jac.T + sensor_var)
    K_k = p_cov_check @ h_jac.T @ np.linalg.inv(h_jac @ p_cov_check @ h_jac.T + np.eye(3)*sensor_var)

    # 3.2 Compute error state
    # print(K_k, y_k, p_check)
    dx_hat = K_k @ (y_k-p_check).T # hk - p_check 
    ...
    p_hat = p_check + p_error
    v_hat = v_check + v_error
    q_hat = Quaternion(axis_angle=q_error).quat_mult_right(q_check) # ?? right?
    # 3.4 Compute corrected covariance
    p_cov_hat = (np.identity(K_k.shape[0]) - K_k @ h_jac) @ p_cov_check
    # print(p_hat, v_hat, q_hat, p_cov_hat)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    #     break
    # finally:
    rotationM = Quaternion(*q_est[k-1]).to_mat()
    # print(rotationM)
    p_est[k] = p_est[k-1] + delta_t * v_est[k-1] + (0.5 * delta_t**2) * (rotationM @ imu_f.data[k-1] + g)
    v_est[k] = v_est[k-1] + delta_t * (rotationM @ imu_f.data[k-1] + g)
    q_est[k] = Quaternion(axis_angle = imu_w.data[k-1] * delta_t).quat_mult_right(q_est[k-1])
    # 1.1 Linearize the motion model and compute Jacobians
    F_k = np.eye(9)
    F_k[0,3] = F_k[1,4] = F_k[2,5] = delta_t
    F_k[:3,6:] =  -0.5 * skew_symmetric(rotationM @ imu_f.data[k-1].reshape((3,1))) * delta_t**2
    F_k[3:6,6:] = -  skew_symmetric(rotationM @ imu_f.data[k-1].reshape((3,1))) * delta_t # skew
    F_k[6:,6:] =  rotationM  @  imu_w.data[k-1].T * delta_t

    # L_k = np.zeros((7,6))
    # L_k[1:4,0:3] = L_k[4:,3:] = np.identity(3) 9,6
    # l_jac[3:, :] = np.eye(6) 
    Q = np.eye(6)
    Q[:3,:3] *= var_imu_f**2
    Q[3:,3:] *= var_imu_w**2
    ...
    # errorstate = F_k  @  errorstate + l_jac @ Q
    
    # 2. Propagate uncertainty
    p_cov[k] = F_k  @  p_cov[k-1]  @  F_k.T + l_jac  @  Q  @ l_jac.T
    # print(p_cov[k])
    # 3. Check availability of GNSS and LIDAR measurements
    # 3.1 data is not available in any of the sensors
    # 3.2 available in both sensors
    try:
        gnssav = imu_f.t[k-1] == gnss.t[gnss_i]
        lidarav = imu_f.t[k-1] == lidar.t[lidar_i]
        if gnssav:            
            y_k = gnss.data[gnss_i].T #+ var_gnss
            sensor_var = var_gnss
            gnss_i+=1
            p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(sensor_var, p_cov[k], y_k, p_est[k], v_est[k], q_est[k])

        elif lidarav:
            # idx = np.where(lidar.t == imu_f.t[k-1]
            y_k = lidar.data[lidar_i].T #+ var_lidar
